      ...
      <svg width={size} height={size} className="-rotate-90">
        <defs>
          <linearGradient id={gid} x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="hsl(198.6 93.2% 59.8%)" />
            <stop offset="55%" stopColor="hsl(258.3 89.7% 66.3%)" />
            <stop offset="100%" stopColor="hsl(172.5 66.1% 50.4%)" />
          </linearGradient>
...
/** 小型统计卡（数值为数字时自动 CountUp + mono 字体） */
export function StatCard({ label, value, sub }: { label: string; value: ReactNode; sub?: string }) {
  return (
    <div className="hover-lift rounded-2xl border bg-card p-4">
      <div className="text-xs text-muted-foreground">{label}</div>
      <div className="mt-1 text-2xl font-semibold tracking-tight">
        {typeof value === 'number' ? <CountUp value={value} className="font-mono tabular-nums" /> : value}
      </div>
